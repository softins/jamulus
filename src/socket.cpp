...
void CSocket::Init ( const quint16 iNewPortNumber, const quint16 iNewQosNumber, const QString& strNewServerBindIP )
{
    ...
    if ( bIsClient )
    {
        if ( iPortNumber == 0 )
        {
            bSuccess = pUdpSocket->bind ( hostAddress, iPortNumber );
        }
        else
        {
            ...
            do
            {
                bSuccess = pUdpSocket->bind ( hostAddress, startingPortNumber + iClientPortIncrement );
    ...
    pUdpSocket->setSocketOption ( QAbstractSocket::TypeOfServiceOption, iQosNumber );
}

// Calling shutdown() works on Linux, Mac and Windows to interrupt the blocking recvfrom()
// in the high-priority receiving thread, but Windows doesn't have the SHUT_RD definition.
// Also, Qt does not have an abstraction of shutdown(), so it is necessary to call the system
// function directly on the socket descriptor.
void CSocket::Shutdown() { ::shutdown ( pUdpSocket->socketDescriptor(), 0 /* SHUT_RD */ ); }

CSocket::~CSocket() { pUdpSocket->close(); }
...
void CSocket::SendPacket ( const CVector<uint8_t>& vecbySendBuf, const CHostAddress& HostAddr )
{
    ...
    if ( iVecSizeOut > 0 )
    {
        ...
        {
            status =
                pUdpSocket->writeDatagram ( (const char*) &( (CVector<uint8_t>) vecbySendBuf )[0], iVecSizeOut, HostAddr.InetAddr, HostAddr.iPort );
...
void CSocket::OnDataReceived()
{
    ...
    */
    ...
    const long iNumBytesRead =
        pUdpSocket->readDatagram ( (char*) &vecbyRecBuf[0], MAX_SIZE_BYTES_NETW_BUF, &RecHostAddr.InetAddr, &RecHostAddr.iPort );
