...
void CServer::OnSendCLProtMessage ( const CHostAddress& InetAddr, CVector<uint8_t> vecMessage, CTcpConnection* pTcpConnection )
{
    // the protocol queries me to call the function to send the message
    // send it through the network
    if ( pTcpConnection )
...
void CServer::OnProtocolCLMessageReceived ( int                 iRecID,
